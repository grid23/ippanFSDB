...
module.exports.DBNode = klass(Node, ReadyStateFul, statics => {
    ...
    options_operators.set("!", (a, b) => a == b || a == "*")
            ...
            value: function(...args){
                ...
                return new Promise((resolve, reject) => {
                    if  ( this.parentNode && !this.validate(opts) ) // root node always validates
                        return reject(`node does not validate meta ${[...opts].map(o => { console.log(o); return `${o.key} => ${o.operand}` }).join(", ")}`)
                    const tree = []
      ...
      , validate: { enumerable: true,
            value: function(opts){
                for ( const {key, operator, operand} of opts )
                  if ( !options_operators.get(operator)(this.meta.get(key), operand) )
                    return false
                return true
            }
        }
