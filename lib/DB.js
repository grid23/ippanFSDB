...
const { VERBOSE, DEBUG } = require("./env")
...
module.exports.DB = klass(EventTarget, ReadyStateFul, statics => {
    ...
    const sync = instance => function(){
        ...
        .then(() => new Promise((resolve, reject) => {
            const tree = new DBTree(this.root, { fruits: this.binaries, leaves: this.parsers, debug: this.debug, verbose: this.verbose } )
            const onend = e => {
    ...
    return {
        constructor: function(dict){
            ...
            const debug = dict.debug || VERBOSE// || opts.get("debug") //TODO option should also be readable from .fsdb file ( see a bit later in the promise chain)
            const verbose = dict.verbose || DEBUG// || opts.get("verbose") //TODO idem
            ...
            dbs.get(this).set("ready", Promise.resolve()
                ...
                .then(opts => new Promise(resolve => {
                    ...
                    mkdir(opts.get("TMP"), mode, err => {
                        ...
                        process.addListener("exit", function(path, verbose){
                            return function(){
                                if ( VERBOSE )
                                  console.log(`[${__filename}] attempting to delete ${path}`)
                                spawn("rm", ["-rf", path])
                            }
                        }(opts.get("TMP")))
                ...
                .then(opts => new Promise(resolve => {
                      ...
                      dbs.get(this).set("debug", debug)
                      dbs.get(this).set("verbose", verbose)
                      dbs.get(this).set("root", opts.get("TARGET"))
              ...
              .then(() => new Promise(resolve => {
                  ...
                  const server = new DBOverUnixSocket({ db: this, debug: this.debug, verbose: this.verbose })
                  resolve()
              }))
              .then(() => sync(this))
              .then(() => new Promise(resolve => {
      ...
      , debug: { enumerable: true,
            get: function(){ return dbs.get(this).get("debug") }
        }
      ...
      , query: { enumerable: true,
            value: function(query, cb){
                ...
                return queue(this, () => new Promise(resolve=> {
                    console.log("__Z, db query", query)

                    if ( !this.tree.nodes.has(query) )
                      return reject(new Error(`nodes ${query}, does not exist`))
      ...
      , verbose: { enumerable: true,
            get: function(){ return dbs.get(this).get("verbose") }
        }
    }
})
