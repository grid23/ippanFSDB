...
const { createReadStream, stat, unlink } = require("fs")
...
module.exports.DBNode = klass(Node, ReadyStateFul, statics => {
    ...
    return {
        constructor: function({ path, realpath, debug=false, verbose=false }, opts){
            ...
            nodes.get(this).set("socket_path", `/tmp/.ippanfsdb/${uuid()}.sock`)
      ...
      , debug: { enumerable: true,
            get: function(){ return nodes.get(this).get("debug") || DEBUG }
        }
      ...
      , read: { enumerable: true,
            value: function(...args){
                console.log("__Y, dbnode read", args)
                if ( VERBOSE )
                  console.log(`[${__filename}] DBNode::read(opts, rules, cb)`, args)
                ...
                return new Promise((resolve, reject) => {
                    if  ( this.parentNode && !this.validate(opts) ) // root node always validates
                        return reject(`[${__filename}] node does not validate meta ${[...opts].map(o => { console.log(o); return `${o.key} => ${o.operand}` }).join(", ")}`)
                    const tree = []
                    ...
                    if ( VERBOSE )
                      console.log(`[${__filename}] DBNode::read() tree => `, tree)

                    console.log("__X, dbnode read, create unix socket server", this.socket_path)
                    const server = new UnixSocketServer(this.socket_path)
                    server.addEventListener("listening", e => {
                        console.log("__W, dbnode read, unix socket server listening")

                        const cp = fork(iocp, [`--socket=${server.socket_path||server.socket}`, `--debug=${this.debug}`, `--verbose=${this.verbose}`])
                        const pid = cp.pid

                        cp.addListener("exit", code => {
                            if ( !server.socket_path && !server.socket )
                              return server.close()

                            if ( VERBOSE )
                              console.log(`[${__filename}] sub task exited ${code}, attempting to delete(${server.socket_path||server.socket})...`)
                            spawn("rm", ["-rf", server.socket_path || server.socket])
                            server.close()
                        })
                ...
                .then(({server, socket, tree, cp}) => new Promise((resolve, reject) => {
                    console.log("__V, dbnode read, socket connected")
                    const op = { op: "read", binary: module.exports.DBNode[binary], tree }

                    const ontextframe = e => {
                        console.log("__U, dbnode read, unix socket message", e.unmask())
                        ...
                        if ( op === "read" )
                          ...
                          reject(new Error(errors.EMPTY_MSG))
                    }
                ...
                .catch(e => {
                    if ( this.debug )
                      console.log(`[${__filename}] <= untreated message (unknown op), ${e.unmask()}`)
                    cb(e, null)
                })
            }
        }
      ...
      , socket_path: { enumerable: true,
            get: function(){ return nodes.get(this).get("socket_path") }
        }
      ...
      , verbose: { enumerable: true,
            get: function(){ return nodes.get(this).get("verbose") || VERBOSE }
        }
