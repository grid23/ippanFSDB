...
module.exports.DBNode = klass(Node, statics => {
    ...
    options_operators.set(null, (a, b) => !b || a == b || b == "*")
    options_operators.set("!", (a, b) => a == b || b == "*")
    options_operators.set("?=", (a, b) => {
        try {
          a = parseInt(a, 10)
          b = parseInt(b, 10)
    ...
    const extractOptions = o => {
        ...
        Object.keys(o)
          ...
          .forEach(([key, value]) => {
              ...
              set.add({ key, value, operator, operand })
          })
    ...
    return {
                ...
                stat(this.metapath, (err, stats) => {
                    ...
                    readFile(this.metapath, (err, buffer) => {
                        ...
                        buffer.toString()
                          ...
                          .forEach(([key, value]) => this.meta.set(key, value))
      ...
      , validate: { enumerable: true,
            value: function(opts){
                for ( const {key, value:to_match, operator, operand} of opts ) {
                    const match = options_operators.get(Operator)(to_match, this.meta.get(key))
                    console.log(key, to_match, this.meta.get(key), match)
                }
